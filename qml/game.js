...
function getScores() {
    var scores = "";
    for (var j=0; j< gameModeKeys.length; j++) {
        scores = scores + gamemodes[gameModeKeys[j]] + ":\n\n";

        for (var i=Object.keys(difficulties).length-1; i >= 0; i--){
            scores = scores + difficulties[i] + '\n' + getHighScoreList(gameModeKeys[j], i);
        }
    }
    return scores;
}
